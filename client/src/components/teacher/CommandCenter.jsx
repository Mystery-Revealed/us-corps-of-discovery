...
// download the PDF → End Session (deletes everything from memory). Corps of
// Discovery is single-role solo (no pick), so students start on their own as
...
const GROUP = 'class';
const TEKS = 'TEKS 8.5C, 8.6B, 8.10A, 8.11A, 8.31B';
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Corps of Discovery: Mission West · {TEKS}</p>
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Corps of Discovery · session-only data · download the PDF before you end</p>
        </div>
...
function downloadPdf(roster, cls) {
  ...
  doc.setFontSize(16);
  doc.text('Corps of Discovery: Mission West — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  doc.text(`8th Grade U.S. History · Corps of Discovery: Mission West · ${TEKS}`, 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`corps-of-discovery_${roster.joinCode}_${date}.pdf`);
}
