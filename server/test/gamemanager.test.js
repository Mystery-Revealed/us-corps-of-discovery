...
// and inspects the emit instructions it returns. No sockets involved. Corps of
// Discovery is single-role solo (no pick, no branch, no AI rival), so these focus
// on the solo lifecycle, the one class-wide group, and the accuracy path.
import test from 'node:test';
import assert from 'node:assert/strict';
import { GameManager } from '../src/GameManager.js';
import game, { phasesFor, SIDE } from '../src/games/usCorpsOfDiscovery.js';
...
test('the default game is Corps of Discovery', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  assert.equal(manager.registry.get(joinCode).gameId, 'us-corps-of-discovery');
});
...
test('a student joins, the match begins on join, and all-right earns 100% and the top ending', () => {
  const manager = new GameManager();
  const joinCode = makeSession(manager);
  const res = join(manager, joinCode, 'Ana');
  ...
  assert.equal(end.length, 1, 'match ends after 12 decisions');
  assert.equal(end[0].payload.you.accuracy, 100);
  assert.equal(end[0].payload.you.ending.key, 'captains');
  assert.match(end[0].payload.you.debrief, /8,000 miles/, 'debrief present');
